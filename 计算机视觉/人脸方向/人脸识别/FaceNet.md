...
## 4.实验 Experiments
我们在四个数据集上评估我们的方法，除了Labelled Faces in the Wild和YouTube Faces外，我们还对人脸验证任务进行了评估。即，给定一对人脸图像，使用平方L2距离阈值D(xi,xj)来确定相同和不同的分类。所有同一身份的面部对（i,j）表示为Psame，而不同身份的所有对表示为Pdiff。 
