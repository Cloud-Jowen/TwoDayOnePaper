...
## 1.介绍 Introduction
年龄估计是计算机视觉中的一个基础问题，近年来受到了广泛关注。然而，对评估过程进行更深入的研究揭示了两个潜在问题。首先，大多数公开数据集没有定义标准化的数据分割，并且使用的数据分割很少公开，导致结果难以重现。其次，新方法通常会修改年龄估计系统的多个组件，使得难以确定哪个修改是性能提升的原因。

本文旨在批判性地分析年龄估计研究中的评估实践，突出问题，并呼吁学术界遵循良好的评估实践来解决这些问题。我们对最近的基于深度学习的面部图像年龄估计方法进行了基准测试和公平比较。我们专注于最先进的方法，它们通过改变通用架构的最后一层或损失函数来适应年龄估计任务。尽管这可能看起来有限制，但需要注意的是，该领域提出的大多数方法都属于这个类别。通过比较只修改网络的一小部分的方法，我们旨在确保公平的评估，因为其余设置可以保持相同。除了通常的类内性能评估外，我们还评估了它们在跨数据集泛化方面的表现，这在年龄预测文献中一直被忽视。令人惊讶的是，我们发现损失函数和决策层对结果的影响，通常是区分不同方法的主要组成部分，与其他因素相比微不足道。
...
<a id="2.1数据分割"></a>
## 2.1 数据分割 Data Splits
聚焦于年龄估计的论文通常使用多个数据集进行测试[1,4,20,21,23,24,29]，其中最常用的是MORPH [23]数据集。然而，这些论文之间的评估程序并不统一。例如，OR-CNN [21]将数据集随机分为两部分：80%用于训练，20%用于测试。没有提到用于模型选择的验证集。随机分割协议也用于[3、5、12、13、19、28]，但由于很少公开，因此具体的数据分割方式在研究之间有所不同。由于数据集中包含同一人的多张图片（许多在同一年龄拍摄），同一个个体可能同时出现在训练集和测试集中。这种重叠会引入偏差，导致评估结果过于乐观。使用随机分割时，数据泄漏的程度会有所不同，使得某些数据分割比其他数据分割更具挑战性。此外，这从根本上改变了整个设置；很少会想要将年龄估计系统部署在训练数据中存在的人员上。因此，比较不同方法并确定哪种方法是基于已发表结果最有效的方法变得困难。

仅有部分论文[22, 27]意识到由分割策略引入的偏差，并通过实施主题独占 (SE) 分割来解决这个问题。这种方法确保一个人的所有图像都独占于 (i) 训练集、(ii) 验证集或者 (iii) 测试集中。

为了评估在MORPH数据集上随机分割 (RS) 的普遍程度，我们对自2013年以来在CVPR和ICCV上发表的所有关于年龄估计的论文进行了调查（见附录）。我们发现了16篇关注年龄估计的论文，其中有九篇使用了RS，两篇使用了SE，五篇使用了专门的分割方式，而三篇则未使用MORPH数据集。我们还对其他研究会议和期刊进行了调查，包括：IJCAI、BMVC、ACCV、IEEE TIP、Pattern Recognit. Lett.、Pattern Anal. Appl.，并找到了八篇有影响力的年龄估计论文使用了MORPH数据集。其中，有七篇使用了RS，一篇使用了专门的分割方式。

总体而言，我们发现只有约10%使用MORPH数据集的论文采用了SE协议。这一发现令人担忧，因为MORPH[23]是用于比较年龄估计方法最流行的数据集。其他数据集也未提供可靠的基准，因为标准化的数据分割仅适用于两个公开的年龄估计数据集：(i) ChaLearn Looking at People Challenge 2016 (CLAP2016) 数据集[1]，该数据集相对较小，仅包含不到8000张图像；(ii) Cross-Age Celebrity Dataset (CACD2000) [4]，该数据集具有嘈杂的训练注释，并非用于年龄估计。因此，仅使用这些数据集进行方法比较也是不令人满意的。其他流行的数据集，如AgeDB数据集[20]和Asian Face Age Dataset (AFAD) [21]，每个人都包含多个图像，需要进行SE分割。然而，它们缺乏得到学术界认可的数据分割，并且通常使用RS协议。因此，它们受到了与MORPH[23]相同的问题的困扰。
