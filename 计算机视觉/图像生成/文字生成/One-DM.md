...
先前的工作 [5，9，13-15，27，46] 介绍了生成对抗网络 (Generative Adversarial Networks) （GAN）（6、16、38）用于手写文本的生成。例如，ScrabbleGAN（13）利用随机噪声作为风格输入，并在字符级别标签上条件内容输入，从而能够合成带有随机采样风格的手写字词。最近的一项研究（11）表明，去噪扩散概率模型（Denoising Diffusion Probabilistic Models，DDPM）（20）、去噪扩散模型（Denoising Diffusion Model，DDM）（37、50、66）等扩散模型甚至比 GAN 更能产生高质量的图像。这促使了诸如 WordStylist（44）、GC-DDPM（12）和 CTIG-DM（70）等方法尝试将固定作者ID条件到去噪过程中以生成具有可控风格的手写文本图像。然而，这些方法的主要局限性是它们无法模仿未见过的作者的书写风格。  
