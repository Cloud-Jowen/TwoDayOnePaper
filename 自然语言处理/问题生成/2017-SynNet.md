...
<a id="4.1Two-StageSynNet"></a>
### 4.1 Two-Stage SynNet

为了启动我们的模型$`f_s`$，我们使用了一个SynNet（图1），它由答案合成模块和问题合成模块组成，在$`p_t`$上生成数据。我们的SynNet学习给定段落$`p`$生成答案$`a = p_{a_{start}},..p_{a_{end}}`$和问题$`q = \left \{ q_0,q_1,...,q_n \right \}`$的条件概率分布$`P\left ( q,a|p \right )`$ 。我们将联合概率分布$`P\left ( q,a|p \right )`$分解为条件概率分布$`P\left ( q|p,a \right ) P\left ( a|p \right ) `$，首先生成答案a，然后根据答案和段落生成问题$`q`$。

<a id="4.1.1答案生成模块"></a>
### 4.1.1 答案生成模块 Answer Synthesis Module

在我们的答案合成模块中，我们训练了一个简单的IOB标记器来预测段落中的每个单词是否为答案的一部分。

更正式地说，给定一个段落中的词集合$` p = \left \{ p_1,...p_n \right \} `$，我们的IOB标记模型学习条件概率标签$`y_1,...,y_n`$, 其中如果在训练集中，标注者为单词$`p_i`$打上了答案标记，则$`y_1 \in IOB_{START},IOB_{MID},IOB_{END}`$；否则为NONE。

我们使用双向长短期记忆网络（Bi-LSTM）进行标记。具体来说，我们将每个词$`p_i -> p_{i}^{*}`$映射到连续的向量空间中，通过预先训练好的 GloVe嵌入（Pennington等人，2014）。然后我们在词嵌入$`p_{1}^{*},...,p_{n}^{*}`$上运行 Bi-LSTM来产生上下文相关的词表示$`h_1,...,h_n`$，将其馈送给两个全连接层，最后加上softmax来为每个词生成我们的标签概率。

我们选择标记器产生的所有连续的 y = NONE 的片段作为我们的候选答案块，并将其馈入问题合成模块以生成问题。
